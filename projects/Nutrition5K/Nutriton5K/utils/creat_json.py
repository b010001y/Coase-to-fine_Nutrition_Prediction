...
def main():
    segment = 18
    del_food = ['dish_1551232973','dish_1551138237','dish_1551389458','dish_1551381990','dish_1560974769','dish_1564159636']
    all_data = ReadCsvData('../../nutrition5k_dataset/metadata/dish_metadata_overhead.csv')
    ...
    for t in CATEGORY:
        ...
        # file = '/disk/btc010001/nutrition5k/data/data_old/mass_overhead.json'
        #
        # with open(file, 'r') as f:
        #     datas = json.load(f)
        # annotations = datas['annotations']

        # train_fun = nutrition5k(dir, annotations, phase='train',category=t)
        # train_loader = torch.utils.data.DataLoader(train_fun, batch_size=1, shuffle=True)
...
def train_test(ann):
    ls = list(ann)
    after_train = []
    with open("../../nutrition5k_dataset/dish_ids/splits/rgb_train_ids.txt", "r") as f:  # 打开文件
    ...
    ls = list(ann)
    after_test = []
    with open("../../nutrition5k_dataset/dish_ids/splits/rgb_test_ids.txt", "r") as f:  # 打开文件
...
def write_json(ls,t,phase,segment):
    with open('../data/Baseline_data/'+'change_'+phase+'_'+t+str(segment)+'.json', 'wb') as f1:
        f1.truncate()
