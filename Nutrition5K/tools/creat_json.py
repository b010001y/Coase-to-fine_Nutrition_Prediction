...
from dataset_mass import nutrition5k
...
def main():
    ...
    all_data = ReadCsvData('/disk/btc010001/nutrition5k/data/nutrition5k_dataset/metadata/dish_metadata_overhead.csv')
...
def train_test(ann):
    ls = list(ann)
    after_train = []
    with open("/disk/btc010001/New_Nutrition5K/nutrition5k_dataset/dish_ids/splits/rgb_train_ids.txt", "r") as f:  # 打开文件
...
def write_json(ls,t,phase,segment):
    with open('/disk/btc010001/New_Nutrition5K/data/Baseline_data/'+'change_'+phase+'_'+t+str(segment)+'.json', 'wb') as f1:
        f1.truncate()
